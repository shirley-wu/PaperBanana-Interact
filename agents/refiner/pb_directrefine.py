...
def format_chat_history(chat_history):
    ...
    for turn in chat_history:
        ret.append("{}: {}".format(turn["role"].capitalize(), turn["content"]))
    return "\n".join(ret)


class PBDirectRefine(BaseAgent):
...
Your task is to conduct a sanity check and provide a critique of the target diagram based on the **user's request**. You are provided with the initial request ('Methodology Section' and 'Figure Caption') and the iterative feedback provided via 'Chat History'. Please preserve the correct elements from previous turns and directly address the feedback given in the **latest** turn of the Chat History.

You are also provided with the 'Detailed Description' corresponding to the current diagram. Please make improvements over the current description.
...
-   **Chat History**: [List of chat utterance from all prior turns. Focus on addressing the **LATEST** turn]
